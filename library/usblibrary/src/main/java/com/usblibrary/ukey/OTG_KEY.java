...
public class OTG_KEY {
	...
	public int UserLogin(String userPin) {
		...
		System.arraycopy(bAuthData, (int) 0, bSendBuf, (int) 2,
				bAuthData.length);
	...
	public byte[] signData(String containerName, int keyType, int hashType,
			byte[] inData, byte[] getSignSendData, int[] getSignSendLen) {
		byte[] bhContainer = new byte[2];
		...
		/*
		 * else { System.arraycopy(bRevBuf, 0, bhContainer, 0,
		 * bhContainer.length); }
		 */
		// Proc 2.1 Update
		...
		if (iAlgType == TYPE_ALG_RSA_1024) {
			...
			System.arraycopy(mAppHandle, 0, bSendBuf, 0, mAppHandle.length);
			System.arraycopy(bhContainer, 0, bSendBuf, 2, bhContainer.length);
			System.arraycopy(bDegistResult, 0, bSendBuf, 4,
					bDegistResult.length);

			length[0] = bRevBuf.length;
			getSignSendLen[0] = bDegistResult.length + 4;
			System.arraycopy(bSendBuf, 4, getSignSendData, 0,
					bDegistResult.length + 4);
			ret = msyUsbBase.scsi_cmd(cmd, bSendBuf, bRevBuf, length);
			if (ret != UsbStatus.RETURN_SUCCESS) {
				return null;
			}
	...
	public boolean importCertificate(String containerName, int keyType,
			byte[] cert) {
	...
	public boolean initDevice(String superPin, String adminPin, String userPin,
			int retryNum) {
		...
		if (bTmpArray.length <= 24) {
			//
			System.arraycopy(bTmpArray, (int) 0, bSuperPin, (int) 0,
					bTmpArray.length);
		} else {
			//
	...
	public int fillPackage(byte[] pData, int packageType, int length,
			byte[] pContent) {
		int totalLen = 0;
		...
		if ((packageType != CMD) && (packageType != DATA)
				&& (packageType != ENDDATA)) {
			return -803;
		}
		length += 2;
	...
	public int fillPackage_new(byte[] pData, int packageType, int length,
			byte[] pContent) {
		int totalLen = 0;
	...
	public int DeCodeUSB(byte[] source, int sourceLen, byte[] destination,
			int[] destinationLength) {
		int i;
		/*
		 * if( (source[0] != 0xef) || (source[1] != 0x01) ){ return -501; }
		 */
	...
	public int EnCode(int addr, byte[] source, int sourceLen,
			byte[] destination, int[] destinationLength) {
		int i = 0, chkSum = 0;
		int ValH, ValL;
	...
	public int PSSearch(int addr, int bufferID, int startPage, int pageNum,
			int[] mbAddr) {
		byte[] cmd = new byte[10];
		int num;
		int result;
		...
		if ((bufferID < 1) || (bufferID > 3) || (startPage < 0)
				|| (pageNum < 0)) {
			return -4;
		}
			...
			if (in[i] == (byte) 0xef) {
				if (in[i + 1] == (byte) 0x01) {
					if (in[i + 6] == (byte) 0x02) {
						...
						// Log.i( TAG2,
						// "upData2Temp 222 data = "+bytesToHexString(tempData));
						...
						System.arraycopy(in, i, tempData, 0, 139);
						// Log.i( TAG2,
						// "upData2Temp 888 data = "+bytesToHexString(tempData));
						System.arraycopy(tempData, 9, out, j * 128, 128);
						break;
					} else {
						k++;
	...
	public int upChar(int iBufferID, byte[] pTemplet, int iTempletLength) {
		...
		/*
		 * int totalLen = iTempletLength; int i = 0; int tempLength = 0; int
		 * nonius = 0; do{ for( i = 0; i<350; i++) { getData[i] = (byte) 0xff; }
		 * ret = RecvCom(getData); if( 0 != ret ) { return -1; } tempLength =
		 * GetPackageContentLength(getData) - 2; //ret =
		 * VerifyResponsePackage(getData[0], getData);
		 * //System.out.println("upChar 333 ret = "+ret); Log.i( TAG2,
		 * "getData = "+bytesToHexString(getData)); if( (nonius+tempLength) >
		 * 512) { Log.i( TAG2,
		 * "nonius+tempLength error nonius="+nonius+" tempLength="+tempLength);
		 * return -1; } System.arraycopy( getData, 9, pTemplet, nonius,
		 * tempLength); nonius += tempLength; }while( getData[6] != ENDDATA );
		 * if( 0 == totalLen) { return -2; }
	...
	public int downChar(int iBufferID, byte[] pTemplet, int iTempletLength) {
		...
		for (j = 0; j < 4; j++) {
			...
			if (j < 3) {
				...
				System.out.println("downChar totalLen=" + totalLen);
				num = fillPackage_new(sendData, DATA, 128, tempTemplet);
				System.out.println("000000");
				ret = SendPackageUDisk(0xffffffff, sendData);
				System.out.println("downChar SendPackageCom 3 ="
	...
	/*
	 * public int PSUpChar( int addr, byte bufferId ) { byte[] cmd = new
	 * byte[10]; int num; int ret; byte[] sendData = new byte[MAX_PACKAGE_SIZE];
	 * 
	 * cmd[0] = UP_CHAR;
	 * 
	 * num = fillPackage(sendData, CMD, 1, cmd); if( (ret =
	 * SendPackageUDisk(addr, sendData)) != 0 ) { return -56; } return
	 * GetPackageImageUDisk( pImageData, IMAGE_X*IMAGE_Y);
	 * 
	 * }
	 */
